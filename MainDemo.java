...
class FogNode {
  ...
  public void createDistanceMatrix(int numberOfFogNodes)// this ,ethod is used to create the distance matrix
  {
    ...
    System.out.println();
    System.out.println("distance matrix has been created");
    System.out.println("distance matrix values are..");
    for (i = 0; i < numberOfFogNodes; i++) {
      for (j = 0; j < numberOfFogNodes; j++) {
  ...
  public void addRequestList(Request r) {// this method is used to add request to the scheduled request list of fog node
    ...
    this.requestCounter++;
    System.out.println("RequestID: " + r.id + " arrived at FognodeID: " + this.id);
  }
  ...
  void doContextSharing(FogNode[] f, int checkCiValue, String appType, double delayTolerance, double expServiceTime) {
    ...
    System.out.println("Broadcasting is done from fog node id:: " + this.id);
      ...
      if (appType.equals("SHC")) {
        ...
        if (getSizeAppSender == checkCiValue) {
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out
                .println(checkCiValue + " number of CI of the SHC appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SHC);
            System.out.println(checkCiValue + " number of CI of SHC appType is received in fog node id:: " + this.id);
          ...
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out
                .println(getSizeAppSender + " number of CI of the SHC appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SHC);
            System.out
                .println(getSizeAppSender + " number of CI of SHC appType is received in fog node id:: " + this.id);
        ...
        if (getSizeAppSender == checkCiValue) {
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out.println(checkCiValue + " number of CI of the SA appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SA);
            System.out.println(checkCiValue + " number of CI of SA appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + checkCiValue; i++)
              this.SA[i] = "c";
          } else
            continue;
          ...
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out
                .println(getSizeAppSender + " number of CI of the SA appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SA);
            System.out
                .println(getSizeAppSender + " number of CI of SA appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + getSizeAppSender; i++) {
              this.SA[i] = "c";
              checkCiValue = checkCiValue - getSizeAppSender;
              continue;
        ...
        if (getSizeAppSender == checkCiValue) {
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out.println(checkCiValue + " number of CI of the SE appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SE);
            System.out.println(checkCiValue + " number of CI of SE appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + checkCiValue; i++)
              this.SE[i] = "c";
          } else
            continue;
          ...
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out
                .println(getSizeAppSender + " number of CI of the SE appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SE);
            System.out
                .println(getSizeAppSender + " number of CI of SE appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + getSizeAppSender; i++) {
              this.SE[i] = "c";
              checkCiValue = checkCiValue - getSizeAppSender;
              continue;
        ...
        if (getSizeAppSender == checkCiValue) {
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out.println(checkCiValue + " number of CI of the ST appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.ST);
            System.out.println(checkCiValue + " number of CI of ST appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + checkCiValue; i++)
              this.ST[i] = "c";
          } else
            continue;
          ...
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out
                .println(getSizeAppSender + " number of CI of the ST appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.ST);
            System.out
                .println(getSizeAppSender + " number of CI of ST appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + getSizeAppSender; i++) {
              this.ST[i] = "c";
              checkCiValue = checkCiValue - getSizeAppSender;
              continue;
        ...
        if (getSizeAppSender == checkCiValue) {
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out.println(checkCiValue + " number of CI of the SV appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SV);
            System.out.println(checkCiValue + " number of CI of SV appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + checkCiValue; i++)
              this.SV[i] = "c";
          } else
            continue;
          ...
          if (delayTolerance >= expServiceTime + f[i].getCommunicationTime()) {
            ...
            System.out
                .println(getSizeAppSender + " number of CI of the SV appType are sent from fog node id:: " + f[i].id);
            getSizeAppReceiver = this.getSize(this.SV);
            System.out
                .println(getSizeAppSender + " number of CI of SV appType is received in fog node id:: " + this.id);
            for (i = getSizeAppReceiver; i <= getSizeAppReceiver + getSizeAppSender; i++) {
              this.SV[i] = "c";
              checkCiValue = checkCiValue - getSizeAppSender;
              continue;
  ...
  int doContextMigration(Request r, int indexOFRequest, FogNode[] f) {
    ...
    Random rand = new Random();
    int nearestFNode = 0, sizeOfApp, checkForCiValue, count = 0, i = 0, minDistance = 0, j = 0, isFailed = 0;
    ...
    while (count != 1) {
      ...
      System.out.println("===========NEAREST FOD NODE SELECTED:: "+ nearestFNode+" ===========");
      ...
      if (schedulingStrValue.equals("SCHEDULE")) {
        ...
        if (r.appType.equals("SHC")) {
          ...
          System.out.println(checkForCiValue + " number of CI of SHC appType is shareed from fog node id::  " + this.id);
        } else if (r.appType.equals("SA")) {
          this.SA = this.deleteNumberOfCi("SA", checkForCiValue);
          System.out.println(checkForCiValue + " number of CI of SA appType fis shareed from fog  nodeid :: " + this.id);
        } else if (r.appType.equals("SE")) {
          this.SE = this.deleteNumberOfCi("SE", checkForCiValue);
          System.out.println(checkForCiValue + " number of CI of SE appType fis shareed from fog  nodeid :: " + this.id);
        } else if (r.appType.equals("ST")) {
          this.ST = this.deleteNumberOfCi("ST", checkForCiValue);
          System.out.println(checkForCiValue + " number of CI of ST appType fis shareed from fog  nodeid :: " + this.id);
        } else if (r.appType.equals("SV")) {
          this.SV = this.deleteNumberOfCi("SV", checkForCiValue);
          System.out.println(checkForCiValue + " number of CI of SV appType fis shareed from fog  nodeid :: " + this.id);
        }

        // Receiving CI in receiver (or this fog node) side
        if (r.appType.equals("SHC")) {
          getSizeAppReceiver = f[nearestFNode].getSize(f[nearestFNode].SHC);
          System.out.println(checkForCiValue + " number of CI of " + r.appType + " appType is received in fog id "+ f[nearestFNode].id);
    ...
    if(isFailed == 1){
      System.out.println("Request ID:: "+r.id+" failed to be executed in Fog.");
      return isFailed;
    }
...
class Request {
  ...
  Request() {
    ...
    this.delayTolerance = rand.nextDouble() * (0.9 - 0.7) + 0.7;
  }
}
public class MainDemo {
  ...
  public static void main(String[] args) {
    ...
    System.out.print("Enter the number of Fog nodes :: ");
    ...
    // Requests getting arrived at different fog nodes based on random topology
    for (int i = 0; i < noOfRequests; i++) {
    ...
    for (int i = 0; i < noOfFogNodes; i++) {
      ...
      System.out.println("Available applications and context instances:: ");
      // System.out.println("Size of SHC"+F[i].getSize(F[i].SHC));
      System.out.print("SHC values:: " + " ");
      for (int j = 0; f[i].SHC[j] != null; j++) {
        System.out.print(f[i].SHC[j] + " ");
      }
      System.out.println("");
      System.out.print("SA values:: " + " ");
      for (int j = 0; f[i].SA[j] != null; j++) {
        System.out.print(f[i].SA[j] + " ");
      }
      System.out.println("");
      System.out.print("SE values:: " + " ");
      for (int j = 0; f[i].SE[j] != null; j++) {
        System.out.print(f[i].SE[j] + " ");
      }
      System.out.println("");
      System.out.print("ST values:: " + " ");
      for (int j = 0; f[i].ST[j] != null; j++) {
        System.out.print(f[i].ST[j] + " ");
      }
      System.out.println("");
      System.out.print("SV values:: " + " ");
      for (int j = 0; f[i].SV[j] != null; j++) {
        System.out.print(f[i].SV[j] + " ");
      }
      System.out.println("");
      System.out.println("Requests arrived:: ");
      for (int j = 0; f[i].scheduledReqList[j] != null; j++) {
        System.out.println("Request Id:: " + " " + f[i].scheduledReqList[j].id);
        System.out.println("App Type:: " + f[i].scheduledReqList[j].appType);
        System.out.println("No. of context instances required: " + f[i].scheduledReqList[j].ciRequired);
        System.out.println("");
        System.out.println("");
      }
    }
        ...
        for (int j = 0; f[i].scheduledReqList[j] != null; j++) {
          if (f[i].scheduledReqList[j].isScheduled == true) {
            ...
            if (checkscheduleval.equals("SCHEDULE")) {
              ...
              System.out.println("");
              System.out.println(
                  "Sharing of CI required for Req ID: " + f[i].scheduledReqList[j].id + " in Fog Id: " + f[i].id);
